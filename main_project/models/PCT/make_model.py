...
@Desc    : 
    本文件组装了以下核心组件：
    1. PolarTransform: 极坐标几何矫正模块 (创新点)
    2. Backbone: ViT-Small (FSRA 版本) 或 VAN
    3. LPN Head: 局部特征分块策略 (Local Part-based Network)
"""
...
class build_transformer(nn.Module):
    ...
    流程:
    Input -> Polar Transform -> ViT Backbone -> Global/Local Features -> Classifiers
    """
    def __init__(self, opt, num_classes, block=4, return_f=False):
        ...
        # =====================================================
        # 【Step 1】初始化 Polar Transform 模块 (几何矫正)
        # =====================================================
        # input_shape 必须与后续 ViT 的输入尺寸一致
        self.polar = PolarTransform(input_shape=(256, 256))
        print('=> PCT-Net: Polar Transform module initialized.')

        # =====================================================
        # 【Step 2】初始化 Backbone (特征提取)
        # =====================================================
    ...
    def forward(self, x):
        ...
        # -----------------------------------------------------
        # 1. 极坐标变换 (Polar Transform)
        # 这一步是 PCT-Net 与传统方法最大的区别
        # 将 "旋转敏感" 的笛卡尔图像 转换为 "旋转无关" 的极坐标图像
        # -----------------------------------------------------
        x = self.polar(x)

        # -----------------------------------------------------
        # 2. 特征提取 (Backbone)
        # features shape: [Batch, N_tokens, 768]
        # features[:, 0] 是 cls_token (全局特征)
        # features[:, 1:] 是 patch_tokens (局部空间特征)
        # -----------------------------------------------------
        features = self.transformer(x)

        # -----------------------------------------------------
        # 3. 处理全局特征 (Global Branch)
        # -----------------------------------------------------
        # 提取 cls_token 并过分类器
        ...
        if self.training:
            # 训练阶段：需要计算所有分支的 Loss
            # y 是一个列表: [局部结果1, 局部结果2, ..., 全局结果]
            y = y + [tranformer_feature]
            if self.return_f:
                cls, feats = [], []
                for i in y:
                    cls.append(i[0]) # logits 用于 CrossEntropy Loss
                    feats.append(i[1]) # features 用于 Triplet Loss
                return cls, feats
        else:
            # 测试阶段：拼接所有特征向量用于检索 (Retrieval)
            # 将全局特征维度对齐 [Batch, Dim, 1]
            tranformer_feature = tranformer_feature.view(tranformer_feature.size(0), -1, 1)
            # 拼接局部特征和全局特征 -> [Batch, Dim, Block+1]
    ...
    def get_heartmap_pool(self, part_features, add_global=False, otherbranch=False):
        ...
        不只是简单地把图片切成几条，而是根据特征的响应强度 (Activation Magnitude)
        对 Patch 进行排序和分组，确保每一组都包含了最有信息量的特征。
        """
